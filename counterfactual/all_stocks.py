"""
Counterfactual sampling: (exactly) 20% higher (log-)returns on all stocks
"""
...
mu_hat = stocks.mean()
mask = jnp.array(
    [jnp.eye(n_stocks)[stocks.columns == stock, :].squeeze() for stock in stocks_label]
)
desired_vals = jnp.array(
    [mu_hat[stock] * (1.2 if mu_hat[stock] >= 0 else 0.8) for stock in stocks_label]
)
...
_, pdlmc_traj = pdlmc_run_chain(
    ...
    init_pdlmc,
    jnp.array(0),
    jnp.zeros(len(stocks_label)).squeeze(),
    lambda x: MuSigma(x.mu, 0.5 * (x.sigma + x.sigma.T)),
)


np.savez(
    os.path.join(CWD, "all_stocks"),
