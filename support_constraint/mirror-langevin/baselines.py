...
class MirrorLangevinSampler:
    ...
    def set_initial_particles(self, particles: torch.Tensor):
        ...
        if particles.shape[0] != self.num_samples:
            raise ValueError(
                "Initialisation doesn't contain the same number " "of particles as expected."
            )
    ...
    def mix(
        ...
        for _ in tqdm(range(num_iters), disable=no_progress):
            ...
            particles_dual = (
                particles_dual - stepsize * grad_potential + math.sqrt(2 * stepsize) * scaled_noise
            )
